...
def enable_dpi_awareness(app, scale):
    ...
    app.window.DPI_X, app.window.DPI_Y, app.window.DPI_scaling = Get_HWND_DPI(
        app.window.winfo_id(), scale
    )
...
def get_scaling(window: tk.Tk) -> float:
    """Gets the current UI scaling value of a tkinter instance. `Returns a float`"""
    return window.call("tk", "scaling")
